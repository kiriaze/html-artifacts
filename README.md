...
Inspired by [ThariqS/html-effectiveness](https://github.com/ThariqS/html-effectiveness) — the argument that spatial information, relationships, and structure are better *shown* than described. An interactive HTML page understood in 5 seconds beats a Markdown wall nobody re-reads.
...
| `code-review` | PR writeup, diff explanation | Margin annotations with severity tags, before/after blocks, file tour, jump links |
...
| `status-report` | Sprint recap, what shipped/slipped | Shipped/slipped sections, pure-CSS bar chart, follow-up checklist |
| `design-explorer` | Component variants, color palette | Click-to-copy swatches, typography specimens, interactive component states |
| `research-explainer` | Feature docs, concept tutorial | TL;DR chip, collapsible steps, tabbed config snippets, hover glossary, FAQ |
| `editor-board` | Triage, feature flags, prompt tuning | Draggable columns, live item counts, copy-as-markdown export |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file=\"$CLAUDE_TOOL_RESULT_FILE_PATH\"; if [[ \"$file\" == */docs/plans/*.md || \"$file\" == */docs/architecture.md || \"$file\" == */docs/*.md ]]; then echo \"⚠  html-artifacts: source doc changed — run 'html-artifacts refresh' to update docs/html/\"; fi"
          }
        ]
      }
    ]
